...
public class KMeans {
	...
	public static class KMeansMapper extends Mapper<LongWritable,Text,IntWritable,Cluster>{
		...
		IOException, InterruptedException{
			...
			try {
				...
				if(id == -1)
					...
					cluster.setSumDistance(distance);
		...
		//find the nearest cluster to this instance
		public int getNearest(Instance instance) throws Exception{
			...
			//every point get its nearest cluster,here we get the sum of distances.every cluster has the same data of SumDistance
			//disum = disum + distance;
			return id;
		}
